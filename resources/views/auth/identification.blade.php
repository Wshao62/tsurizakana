...
<div class="layout">
    <div class="title">
    <h2>本人確認書類</h2>
    <p class="font_avenirnext">IDENTIFICATION</p>
    </div>
    <p class="updoc_desc">
    アップロードする書類を選択してください。<br>
    氏名、住所が、<br class="sp">釣魚商店アカウントにご登録いただいた<br class="sp">内容と<br class="pc">一致していることを確認してください。
    </p>
    <form method="POST" action="{{ url('/identification') }}" onsubmit="return doSubmit(this);" enctype="multipart/form-data">
        @csrf
        <div class="updoc_box">
        @if ($errors->count() > 0)
        ...
        @endif
        <p class="upbx_medium upmed1">本人確認書類<span>必須</span></p>
        <select class="select_docs" name="type" required>
            <option value="" style="display: none;">1つ選択してください</option>
            @foreach (\App\Models\User\IdentificationDoc::TYPES_NAME as $_key => $_val)
            <option value="{{ $_key }}" @if(old('type') == $_key) selected @endif>{{ $_val }}</option>
            @endforeach
        </select>
        <div class="clear"></div>
