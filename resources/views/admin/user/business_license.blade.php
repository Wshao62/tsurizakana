...
@section('page_name', '営業許可証確認 | ユーザー管理' )
...
<div class="layout">
    ...
    <div class="row w-100 mt-2">
        <div class="col-4">
            <div class="col-12">
                <img class="img-responsive w-100" src="{{ $business_license->getUrl(1) }}" data-toggle="modal"
                    data-target="#modal1">
            </div>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-body">
                            <img src="{{ $business_license->getUrl(1) }}" class="img-responsive w-100">
                        </div>
                    </div>
                </div>
            </div>
            @if (!empty($business_license->file_path2))
            <div class="col-12 mt-3">
                <img class="img-responsive w-100" src="{{ $business_license->getUrl(2) }}" data-toggle="modal"
                    data-target="#modal2">
            </div>
            <div id="modal2" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-body">
                            <img src="{{ $business_license->getUrl(2) }}" class="img-responsive w-100">
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    function approve()
    {
        ...
        return confirm("営業許可証書類を承認します。この操作は取り消せません。\n実行してよろしいですか？");
    }

    function deny()
    {
        $('input[name="judge"]:eq(1)').prop('checked', true);
        return confirm("営業許可証書類を否認します。この操作は取り消せません。ユーザに通知のメールが飛び再度申請するように促されます。\n実行してよろしいですか？");
